...
def check_file_upload(request):
    ...
    if (ext == 'js'):
        prediction_without_pca = model_js.predict_without_pca(extractor.extract_function_names())
    else:
        prediction_without_pca = model_ps.predict_without_pca(extractor.extract_function_names())
